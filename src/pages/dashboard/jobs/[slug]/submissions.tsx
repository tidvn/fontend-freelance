...
function JobSubmissions({ slug }: Props) {
  ...
  const getJob = async () => {
    ...
    try {
      ...
      getSubmissions(jobDetails.data.id);
  ...
  useEffect(() => {
    if (userInfo?.currentCompanyId && !isJobLoading) {
      getSubmissions();
    }
  }, [skip]);
        ...
        <>
          {isOpen && (
            <PublishResults
              ...
              jobId={job?.id}
              isDeadlinePassed={dayjs().isAfter(job?.deadline)}
              hasWinnersAnnounced={job?.isWinnersAnnounced}
            />
          )}
          <Box mb={4}>
          ...
          {!submissions?.length ? (
            <ErrorSection
              ...
              message="View your job submissions here once they are submitted"
            />
          ) : (
                    ...
                    <Flex align="center" justify={'flex-end'} gap={2} w="full">
                      {selectedSubmission?.isWinner &&
                        ...
                        !selectedSubmission?.isPaid && (
                          <Tooltip
                            bg={'brand.purple'}
                            hasArrow={true}
                            isDisabled={!!job?.isWinnersAnnounced}
                      ...
                      >
                        ...
                        >
                          ...
                          {selectedSubmission?.isWinner &&
                            rewards.length <
                              Object.keys(job?.rewards || {})?.length && (
                              <option
